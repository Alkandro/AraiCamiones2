...
const Enrique = () => {
  ...
  const menu = getMenu("enrique", "mensaje");
  const { seleccionarPlatillo } = useContext(PedidoContext);
  const navigation = useNavigation();
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    const unsub = obtenerProductos("enrique", "mensaje");
    return () => unsub();
  }, []);
  ...
  const handleSwitchChange = async (platilloId, value) => {
    if (!isSwitchDisabled[platilloId]) {
      ...
      try {
        await firebase.db.collection("enriqueMensaje").doc(platilloId).update({
          leido: isLeido,
        });
        if (isLeido) {
  ...
  const eliminarProducto = async (platilloId) => {
    try {
      await eliminarProductoFirebase("enrique", "mensaje", platilloId);
      obtenerProductos();
    } catch (error) {
