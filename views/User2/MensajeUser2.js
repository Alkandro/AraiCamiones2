...
import firebaseContextUser2Mensaje from "../../context/firebase/User2State/FirebaseStateUser2Mensaje/firebaseContextUser2Mensaje";
...
const User2 = () => {
  ...
  const handleSwitchChange = async (platilloId, value) => {
    if (!isSwitchDisabled[platilloId]) {
      ...
      try {
        await firebase.db.collection("user2Mensaje").doc(platilloId).update({
          leido: isLeido,
        });
        if (isLeido) {
