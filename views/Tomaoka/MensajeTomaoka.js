...
import firebaseContextTomaokaMensaje from "../../context/firebase/TomaokaState/FirebaseStateTomaokaMensaje/firebaseContextTomaokaMensaje";
...
const Tomaoka = () => {
  ...
  const handleSwitchChange = async (platilloId, value) => {
    if (!isSwitchDisabled[platilloId]) {
      ...
      try {
        await firebase.db.collection("tomaokaMensaje").doc(platilloId).update({
          leido: isLeido,
        });
        if (isLeido) {
