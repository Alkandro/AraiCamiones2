...
import firebaseContextOkamotoMensaje from "../../context/firebase/OkamotoState/FirebaseStateOkamotoMensaje/firebaseContextOkamotoMensaje";
...
const Okamoto = () => {
  ...
  const handleSwitchChange = async (platilloId, value) => {
    if (!isSwitchDisabled[platilloId]) {
      ...
      try {
        await firebase.db.collection("okamotoMensaje").doc(platilloId).update({
          leido: isLeido,
        });
        if (isLeido) {
