...
import firebaseContextUser3Mensaje from "../../context/firebase/User3State/FirebaseStateUser3Mensaje/firebaseContextUser3Mensaje";
...
const User3 = () => {
  ...
  const handleSwitchChange = async (platilloId, value) => {
    if (!isSwitchDisabled[platilloId]) {
      ...
      try {
        await firebase.db.collection("user3Mensaje").doc(platilloId).update({
          leido: isLeido,
        });
        if (isLeido) {
