...
import FirebaseContextUser3Sabado from "./firebaseContextUser3Sabado";

import { OBTENER_PRODUCTOS_USER3 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser3Sabado = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user3Sabado").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser3Sabado.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser3Sabado;
