...
import FirebaseContextUser3Domingo from "./firebaseContextUser3Domingo";

import { OBTENER_PRODUCTOS_USER3 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser3Domingo = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user3Domingo").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser3Domingo.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser3Domingo;
