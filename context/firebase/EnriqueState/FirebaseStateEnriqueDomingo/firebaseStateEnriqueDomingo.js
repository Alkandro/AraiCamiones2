...
import FirebaseContextEnriqueDomingo from "./firebaseContextEnriqueDomingo";

import { OBTENER_PRODUCTOS_ENRIQUE } from "../../../../types";
import _ from "lodash";


const FirebaseStateEnriqueDomingo = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("enriqueDomingo").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextEnriqueDomingo.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateEnriqueDomingo;
