...
import FirebaseContextEnriqueSabado from "./firebaseContextEnriqueSabado";

import { OBTENER_PRODUCTOS_ENRIQUE } from "../../../../types";
import _ from "lodash";


const FirebaseStateEnriqueSabado = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("enriqueSabado").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextEnriqueSabado.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateEnriqueSabado;
