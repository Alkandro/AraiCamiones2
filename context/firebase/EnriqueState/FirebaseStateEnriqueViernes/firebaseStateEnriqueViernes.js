...
import FirebaseContextEnriqueViernes from "./firebaseContextEnriqueViernes";

import { OBTENER_PRODUCTOS_ENRIQUE } from "../../../../types";
import _ from "lodash";


const FirebaseStateEnriqueViernes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("enriqueViernes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextEnriqueViernes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateEnriqueViernes;
