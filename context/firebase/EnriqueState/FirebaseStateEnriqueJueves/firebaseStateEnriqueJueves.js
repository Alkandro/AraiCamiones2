...
import FirebaseContextEnriqueJueves from "./firebaseContextEnriqueJueves";

import { OBTENER_PRODUCTOS_ENRIQUE } from "../../../../types";
import _ from "lodash";


const FirebaseStateEnriqueJueves = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("enriqueJueves").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextEnriqueJueves.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateEnriqueJueves;
