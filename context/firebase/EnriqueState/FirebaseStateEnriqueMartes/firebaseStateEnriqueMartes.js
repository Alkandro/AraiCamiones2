...
import FirebaseContextEnriqueMartes from "./firebaseContextEnriqueMartes";

import { OBTENER_PRODUCTOS_ENRIQUE } from "../../../../types";
import _ from "lodash";


const FirebaseStateEnriqueMartes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("enriqueMartes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextEnriqueMartes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateEnriqueMartes;
