...
import FirebaseContextUser2Viernes from "./firebaseContextUser2Viernes";

import { OBTENER_PRODUCTOS_USER2 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser2Viernes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user2Viernes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser2Viernes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser2Viernes;
