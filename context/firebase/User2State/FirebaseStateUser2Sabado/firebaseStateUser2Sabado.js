...
import FirebaseContextUser2Sabado from "./firebaseContextUser2Sabado";

import { OBTENER_PRODUCTOS_USER2 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser2Sabado = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user2Sabado").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser2Sabado.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser2Sabado;
