...
import FirebaseContextUser2Martes from "./firebaseContextUser2Martes";

import { OBTENER_PRODUCTOS_USER2 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser2Martes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user2Martes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser2Martes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser2Martes;
