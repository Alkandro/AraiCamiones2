...
import FirebaseContextHoshinoMensaje from "./firebaseContextHoshinoMensaje";

import { OBTENER_PRODUCTOS_HOSHINO } from "../../../../types";
import _ from "lodash";

const FirebaseStateHoshinoMensaje = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("hoshinoMensaje").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextHoshinoMensaje.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateHoshinoMensaje;
