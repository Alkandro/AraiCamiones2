...
import FirebaseContextYamakadoSabado from "./firebaseContextYamakadoSabado";

import { OBTENER_PRODUCTOS_YAMAKADO } from "../../../../types";
import _ from "lodash";


const FirebaseStateYamakadoSabado = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("yamakadoSabado").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextYamakadoSabado.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateYamakadoSabado;
