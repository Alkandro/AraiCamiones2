...
import FirebaseContextYamakadoDomingo from "./firebaseContextYamakadoDomingo";

import { OBTENER_PRODUCTOS_YAMAKADO } from "../../../../types";
import _ from "lodash";


const FirebaseStateYamakadoDomingo = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("yamakadoDomingo").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextYamakadoDomingo.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateYamakadoDomingo;
