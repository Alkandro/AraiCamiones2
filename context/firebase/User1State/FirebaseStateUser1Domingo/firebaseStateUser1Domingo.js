...
import FirebaseContextUser1Domingo from "./firebaseContextUser1Domingo";

import { OBTENER_PRODUCTOS_USER1 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser1Domingo = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user1Domingo").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser1Domingo.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser1Domingo;
