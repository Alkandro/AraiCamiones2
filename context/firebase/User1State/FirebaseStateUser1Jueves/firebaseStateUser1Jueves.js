...
import FirebaseContextUser1Jueves from "./firebaseContextUser1Jueves";

import { OBTENER_PRODUCTOS_USER1 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser1Jueves = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user1Jueves").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser1Jueves.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser1Jueves;
