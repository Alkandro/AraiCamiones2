...
import FirebaseContextUser1Martes from "./firebaseContextUser1Martes";

import { OBTENER_PRODUCTOS_USER1 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser1Martes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user1Martes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser1Martes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser1Martes;
