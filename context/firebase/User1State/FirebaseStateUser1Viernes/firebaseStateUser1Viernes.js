...
import FirebaseContextUser1Viernes from "./firebaseContextUser1Viernes";

import { OBTENER_PRODUCTOS_USER1 } from "../../../../types";
import _ from "lodash";


const FirebaseStateUser1Viernes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("user1Viernes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextUser1Viernes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateUser1Viernes;
