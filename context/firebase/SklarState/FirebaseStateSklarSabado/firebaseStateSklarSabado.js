...
import FirebaseContextSklarSabado from "./firebaseContextSklarSabado";

import { OBTENER_PRODUCTOS_SKLAR } from "../../../../types";
import _ from "lodash";


const FirebaseStateSklarSabado = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("sklarSabado").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextSklarSabado.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateSklarSabado;
