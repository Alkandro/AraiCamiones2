...
import FirebaseContextSklarJueves from "./firebaseContextSklarJueves";

import { OBTENER_PRODUCTOS_SKLAR } from "../../../../types";
import _ from "lodash";


const FirebaseStateSklarJueves = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("sklarJueves").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextSklarJueves.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateSklarJueves;
