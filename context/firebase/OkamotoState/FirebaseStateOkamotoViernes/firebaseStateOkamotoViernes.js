...
import FirebaseContextOkamotoViernes from "./firebaseContextOkamotoViernes";

import { OBTENER_PRODUCTOS_OKAMOTO } from "../../../../types";
import _ from "lodash";


const FirebaseStateOkamotoViernes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("okamotoViernes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextOkamotoViernes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateOkamotoViernes;
