...
import FirebaseContextOkamotoMiercoles from "./firebaseContextOkamotoMiercoles";

import { OBTENER_PRODUCTOS_OKAMOTO } from "../../../../types";
import _ from "lodash";


const FirebaseStateOkamotoMiercoles = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("okamotoMiercoles").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextOkamotoMiercoles.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateOkamotoMiercoles;
