...
import FirebaseContextOkamotoJueves from "./firebaseContextOkamotoJueves";

import { OBTENER_PRODUCTOS_OKAMOTO } from "../../../../types";
import _ from "lodash";


const FirebaseStateOkamotoJueves = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("okamotoJueves").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextOkamotoJueves.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateOkamotoJueves;
