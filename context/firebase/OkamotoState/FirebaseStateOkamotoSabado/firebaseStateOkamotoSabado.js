...
import FirebaseContextOkamotoSabado from "./firebaseContextOkamotoSabado";

import { OBTENER_PRODUCTOS_OKAMOTO } from "../../../../types";
import _ from "lodash";


const FirebaseStateOkamotoSabado = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const obtenerProductos = () => {
   ...
   const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("okamotoSabado").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextOkamotoSabado.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateOkamotoSabado;
