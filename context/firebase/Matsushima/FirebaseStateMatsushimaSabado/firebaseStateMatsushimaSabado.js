...
import FirebaseContextMatsushimaSabado from "./firebaseContextMatsushimaSabado";
...
const FirebaseStateMatsushimaSabado = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("matsushimaSabado").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextMatsushimaSabado.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateMatsushimaSabado;
