...
import FirebaseContextMatsushimaJueves from "./firebaseContextMatsushimaJueves";
...
const FirebaseStateMatsushimaJueves = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("matsushimaJueves").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextMatsushimaJueves.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateMatsushimaJueves;
