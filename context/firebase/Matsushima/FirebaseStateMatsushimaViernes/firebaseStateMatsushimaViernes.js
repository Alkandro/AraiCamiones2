...
import FirebaseContextMatsushimaViernes from "./firebaseContextMatsushimaViernes";
...
const FirebaseStateMatsushimaViernes = (props) => {
  // Crear state inicial
  const initialState = {
    menu: [],
  };
  ...
  const eliminarProductoFirebase = async (id) => {
    try {
      await firebase.db.collection("matsushimaViernes").doc(id).delete(); // Eliminar el documento por ID
  ...
  return (
    <FirebaseContextMatsushimaViernes.Provider
      value={{
        menu: state.menu,
        firebase,
        obtenerProductos,
...
export default FirebaseStateMatsushimaViernes;
